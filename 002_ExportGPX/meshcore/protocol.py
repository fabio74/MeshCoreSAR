...
class SentResponse:
    send_type: int
    expected_ack_or_tag: bytes
    suggested_timeout_ms: int


def decode_sent_response(frame: bytes) -> SentResponse:
    """Decode RESP_CODE_SENT (0x06).

    Format: code, type, expected_ack_or_tag[4], suggested_timeout uint32 LE.
    """
    if len(frame) < 10 or frame[0] != RESP_CODE_SENT:
        raise ValueError("Non è una RESP_CODE_SENT valida.")
    return SentResponse(
        send_type=frame[1],
        expected_ack_or_tag=frame[2:6],
        suggested_timeout_ms=struct.unpack_from("<I", frame, 6)[0],
    )
